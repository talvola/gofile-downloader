...
**Website token:** The `X-Website-Token` header is a SHA-256 of `"{UA}::en-US::{token}::{time_slot}::{salt}"` where `time_slot = int(time.time() / 14400)`. Reverse-engineered from `https://gofile.io/dist/js/wt.obf.js` (`generateWT` function). The salt rotates when gofile updates the file — if API calls start returning `error-wrongToken` **or `error-notPremium`** (since ~2026-06 a stale salt is reported as notPremium) on content that opens fine in a browser, re-extract: fetch `wt.obf.js`, patch `_sha256` to log its input, call `generateWT(anyToken)` in Node.js, and read the last `::` segment. Current salt as of 2026-06-12: `9844d94d963d30`. UA must match `DEFAULT_UA` in `api_client.py` (currently Firefox 151.0).
