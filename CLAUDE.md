...
**4chan error bodies (`_get_json` in tg_pdf_thread.py):** 4chan serves a site-wide outage as a plain-text body ("Performing maintenance. We'll be back soon.") under HTTP **429** — the same status as real per-IP throttling, and on every endpoint. The status alone can't tell "the site is down, wait it out" from "you're polling too fast, back off", so `_get_json` folds a short body snippet into the re-raised `HTTPError` (`… — server said: …`), which every existing `print(f"...{e}")` call site picks up for free. The re-raise passes `response=r` so `fetch_thread`'s 404 check still works. `_error_body` strips tags from HTML error pages (Cloudflare block pages), caps at `MAX_ERROR_BODY`, and forces ASCII — an unescaped byte would otherwise raise `UnicodeEncodeError` on the cp1252 console and turn a diagnostic into a crash.

**Gofile ID lengths:** IDs were 6 alnum chars historically; since ~2026 new ones are 8 (`gofile.io/d/ILUoTvWO`). Old 6-char links still resolve, so both lengths must keep working. `gofile_dl.py`'s `parse_content_id` is length-agnostic (`[A-Za-z0-9]+`) and needs nothing; only the /tg/ scraper's matchers are length-bound, via `ID_CHARS` in `tg_pdf_thread.py`. Match exactly 6 *or* 8 (`(?:[A-Za-z0-9]{8}|[A-Za-z0-9]{6})`), never a `{6,}` range — a range swallows other services' long IDs and re-opens the false-positive problem the anchors exist to prevent.

**Gofile ID extraction (tg_pdf_thread.py):** Three matchers, all built on `ID_CHARS`. `GOFILE_DFORM_RE` accepts `d/<id>` anchored on whitespace/slash/start — covers `gofile.io/d/…`, `/d/…`, obfuscated `g0f1le /d/…`, and the leading-slash-dropped `d/GDeWzi`; its trailing `(?![A-Za-z0-9])` guard rejects both other services' long IDs (Google Drive's `/d/<33 chars>`) and off-length slugs (7, 9+). `GOFILE_BARE_RE` accepts a lone `/<id>` ONLY when whitespace-/line-bounded on both sides — without those anchors every mid-URL path segment (e.g. `drivethrurpg.com/product/564646/…`) matches as a false gofile ID. Don't relax the bare-slug anchors; on one /tg/ thread they were the difference between 2 real IDs and 12 junk ones. Every accepted bare slug prints a `note:` line for eyeballing. `GOFILE_HYPHEN_RE` accepts the hyphenated `gf-d-<id>` form (hyphens standing in for slashes, first seen in post 98671658), case-insensitive on the prefix and alnum-bounded on both sides; the prefix is specific enough that it prints no note.
